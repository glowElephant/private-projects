...
| **지식 그래프** | 클래스 관계, 의존성, 에러→해결법 | **1,904 노드, 2,574 엣지** |
...
4. Skills, Agents 디렉토리 동기화 (`dotfiles/skills/`, `agents/` → `~/.claude/`)
5. `.env` 파일 생성 (OpenAI API 키)
6. Python 의존성 설치 (openai, faiss-cpu, networkx, pyvis, mcp, rank_bm25)
7. MCP 서버 등록
...
2. 새 pip 의존성 자동 설치 (누락 패키지 감지)
3. `dotfiles/` → `~/.claude/` 설정 동기화 (settings, skills, agents)
4. 증분 업데이트 → 내 세션을 공유 인덱스에 추가
5. `git commit & push` → 다른 PC에 공유
...
- `skills/` — 커스텀 Skills (unity-compile-check, profiler-analysis, ui-toolkit-guide)
...
│   ├── settings.json               # ~/.claude/settings.json 원본 (권한/Hook/StatusLine)
