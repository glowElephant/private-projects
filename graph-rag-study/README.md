...
[Reranking] 벡터 유사도 + 키워드 매칭 + 소스 가중치로 재정렬
  │
  ▼
[BM25 보충] 벡터가 놓친 숫자/식별자/한글 키워드 매칭
  │
  ▼
...
| 소스 | 내용 | 규모 |
|------|------|------|
| 세션 이력 | Claude Code 과거 대화 (Windows + WSL) | 1,649개 세션, 54,000+ 청크 |
...
| **지식 그래프** | 클래스 관계, 의존성, 에러→해결법 | **9,556 노드, 12,063 엣지** |
...
| 임베딩 | OpenAI text-embedding-3-small | 1536차원, 검색 품질 최고, 비용 ~$0.02/100만 토큰 |
| 그래프 추출 | Claude (직접 API) | Anthropic에는 임베딩 API 없음, 텍스트 생성만 가능 |
| 키워드 검색 | rank_bm25 (BM25Okapi) | 벡터 검색 보충, 숫자/식별자 매칭 |
...
**`sync.py` 하나로 셋업과 동기화를 모두 처리합니다.** 이미 설정된 항목은 자동 스킵.

첫 실행 시 (자동 셋업):
1. PC 프로필 감지 + `config.local.json` 생성 (hostname 자동 감지, 미등록 PC 자동 생성)
2. WSL 감지 → 세션 경로 자동 추가
3. Git LFS 초기화 (대용량 벡터DB 파일 관리)
4. Python/Node.js 의존성 설치
5. MCP 서버 등록 (rag-search, computer-control, web-fetcher)
6. `.env` 파일 생성 (필요 시)
7. 외부 도구 설치 (snap-path, Roboto Mono 폰트)
8. 바탕화면 바로가기 생성

이후 실행 시 (동기화만):
1. `git pull` → 다른 PC의 최신 데이터 받기
2. `dotfiles/` ↔ `~/.claude/` 양방향 설정 동기화 (settings, skills, agents, bash)
3. 증분 업데이트 → 내 세션을 공유 인덱스에 추가
4. `git commit & push` → 다른 PC에 공유

> `setup.py`는 독립 셋업용으로 유지 (대화형 모드, `--pc`/`--key` 인자 지원)

### 최초 인덱싱 (data/가 비어있을 때만)

이미 data/가 git에 포함되어 있으므로 (벡터DB는 Git LFS), clone하면 인덱스도 함께 받아집니다.
**새로 인덱싱할 필요가 없습니다.** 바로 사용 가능.

만약 처음부터 다시 빌드하고 싶다면:
```bash
python rag-system/preprocessor.py      # 세션 전처리
python rag-system/indexer.py            # 벡터 인덱싱 (OpenAI embedding)
python rag-system/graph_extractor.py    # 그래프 추출 (Claude API)
```
...
- `settings.json` — Claude Code 설정 (권한, Hook, StatusLine 포함)
- `keybindings.json` — 키바인딩
- `skills/` — 커스텀 Skills (compile-check, profiler-analysis, ui-toolkit-guide, claude-news, sync)
- `agents/` — 커스텀 Subagents (code-reviewer)

수정하면 `sync.py`가 자동으로 커밋+푸시하고, 다른 PC에서 sync하면 반영됩니다.
`enabledPlugins`에 등록된 플러그인은 Claude Code가 세션 시작 시 자동 설치합니다.
...
```
graph-RAG-study/
├── sync.py                         # 셋업 + 동기화 통합 (이것만 실행)
├── setup.py                        # 독립 셋업 (대화형/비대화형, 하위 호환)
├── .gitattributes                  # Git LFS 트래킹 (vector_db)
├── dotfiles/                       # 글로벌 Claude Code 설정 (3대 PC 공유)
│   ├── CLAUDE.md                   # ~/.claude/CLAUDE.md 원본
│   ├── settings.json               # ~/.claude/settings.json 원본 (권한/Hook/플러그인/StatusLine)
│   ├── keybindings.json            # ~/.claude/keybindings.json 원본
│   ├── bash/                       # Git Bash 설정 (→ ~/)
│   │   ├── .bashrc                 # 프롬프트 테마 + 코끼리 인트로
│   │   ├── .minttyrc               # 터미널 색상/폰트
│   │   └── .elephant_walk.py       # 코끼리 워킹 애니메이션
│   ├── skills/                     # 커스텀 Skills (→ ~/.claude/skills/)
│   └── agents/                     # 커스텀 Subagents (→ ~/.claude/agents/)
├── rag-system/                     # 핵심 코드
...
│   ├── vector_db/                  # FAISS 인덱스 + 청크 (Git LFS)
...
- [x] **Step 5: BM25 키워드 보충** - 벡터가 놓치는 숫자/식별자/한글 매칭, Hit Rate 100% 달성

## 온톨로지 (그래프 스키마)

**노드 유형 (10종, 허용 목록 외 사용 금지):**
class, field, method, component, file, enum, shader, feature, error, solution

**관계 유형 (10종, 허용 목록 외 사용 금지):**
has_field, depends_on, uses, inherits, requires, controls, related_to, solved_by, caused_by, modified

**품질 관리:** LLM 프롬프트에 온톨로지 강제, 허용 외 유형 자동 매핑, C# 기본 타입 자동 제거, unknown 0개

**현재 통계:** 9,556 노드, 12,063 엣지

## 벤치마크

20개 질문 × 3조건 비교 (벡터만 / 벡터+Rerank / rag_agent):

| 조건 | Hit Rate | 응답 시간 |
|------|---------|----------|
| 벡터만 | 82.3% | 329ms |
| 벡터+Rerank | 88.5% | 144ms |
| **rag_agent** | **100%** | 835ms |
...
| 작업 | 비용 | 빈도 |
|------|------|------|
| 초기 벡터 인덱싱 (54,000+청크) | ~$0.001 (OpenAI embedding, 사실상 무료) | 1회 |
| 초기 그래프 추출 | Claude API 비용 | 1회 |
| 증분 업데이트 (세션 1개) | ~$0.001 (임베딩 + 그래프 API) | 수시 |
| 검색 쿼리 1회 | ~$0.000001 (OpenAI embedding) | 매 질문 |
